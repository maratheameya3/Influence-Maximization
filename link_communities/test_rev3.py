...
def get_partitions_mod_optimization(g):
    udg = g.copy()
    udg.to_undirected()
    partitions = udg.community_multilevel()
    return partitions

def get_partitions_hierarchical_clustering(g):
    udg = g.copy()
    udg.to_undirected()
    partitions = udg.community_fastgreedy().as_clustering()
    return partitions

def get_partitions_spectral_clustering(g):
    udg = g.copy()
    udg.to_undirected()
    adj_matrix = g.get_adjacency().data
    sc = SpectralClustering(n_clusters=2, affinity='precomputed').fit(adj_matrix)
    labels = sc.labels_
    partitions = [[] for _ in range(4)]
    for label, part in enumerate(labels):
        partitions[part].append(g.vs[label].index)
    return partitions

def get_partitions_stochastic_block(g):
    udg = g.copy()
    udg.to_undirected()
    adj_matrix = g.get_adjacency().data
    bgm = BayesianGaussianMixture(n_components=2, covariance_type='full').fit(adj_matrix)
    labels = bgm.predict(adj_matrix)
    partitions = [[] for _ in range(4)]
    for label, part in enumerate(labels):
        partitions[part].append(g.vs[label].index)
    return partitions

def get_partitions_link_partitioning(g):
    udg = g.copy()
    udg.to_undirected()
    adj_matrix = g.get_adjacency().data
    kmeans = KMeans(n_clusters=2).fit(adj_matrix.reshape(-1, 1))
...
def split_and_calculate(g, loc, k, filename):
    g.vs["color"] = "blue"
    partitions = get_partitions_spectral_clustering(g)
    final_influencers = []
...
split_and_calculate(g, "0", 45, "../data/0.edges")
